  ...
  constructor(conversationReferences) {
      ...
      for (let cnt = 0; cnt < membersAdded.length; cnt++) {
        if (membersAdded[cnt].id !== context.activity.recipient.id) {
          const welcomeMessage =
            "Welcome to the RoomChecker Bot. This bot sends a notification when a user books a room (message everyone who has previously messaged this bot.)"; // Navigate to http://c45e1d71.ngrok.io/api/notify to proactively message everyone who has previously messaged this bot.
    ...
    this.onMessage(async (context, next) => {
      ...
      switch (text) {
        ...
        default:
          //code to handle the submit action of Adaptive card
          if (
            typeof context.activity.value !== "undefined" &&
            context.activity.value.type === "book"
          ) {
